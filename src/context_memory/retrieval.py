...
class TemporalQueryResolver:
    ...
    def resolve(self, question: str, question_date: datetime) -> DateRange:
        with timed_operation(logger, "retrieval.phase0.temporal_resolver", {"question_len": len(question)}) as ctx:
            system_prompt = (
                f"You are a temporal query resolver. Current reference time is {question_date.isoformat()}. "
                "Analyze the question and extract world-validity time range [valid_from, valid_to] if explicit. "
                "If no explicit time range is referenced, return null for both."
            )
            try:
                response = self._llm.structured_completion(system_prompt, question, DateRange)
                if isinstance(response, DateRange):
                    if response.valid_from:
                        response.valid_from -= timedelta(days=2)
                    if response.valid_to:
                        response.valid_to += timedelta(days=2)
...
class QueryRewriter:
    ...
    def rewrite(self, question: str) -> QueryRewriterOutput:
        with timed_operation(logger, "retrieval.phase0.query_rewriter", {"question_len": len(question)}) as ctx:
            system_prompt = (
                "You are a query expansion assistant. Decompose complex questions into atomic sub-questions "
                "and extract key entity/predicate synonyms to maximize keyword recall."
            )
            try:
                response = self._llm.structured_completion(system_prompt, question, QueryRewriterOutput)
...
class HybridRetrievalEngine:
    ...
    def retrieve_and_answer(self, context_id: str, question: str, question_date: datetime, top_k: int = 15) -> str:
        with timed_operation(logger, "retrieval.retrieve_and_answer", {"context_id": context_id, "question_len": len(question)}) as ctx:
            ...
            resolver = TemporalQueryResolver(self._llm)
    ...
    def _hydradb_graph_expansion(
        ...
        with timed_operation(logger, "retrieval.phase2.graph_expansion", {"context_id": context_id, "seed_count": len(seed_facts)}) as ctx:
            ...
            # 1. Fetch node bitemporal properties and connected entities
            cypher = """
            UNWIND $fact_keys AS f_key
            MATCH (f:Fact {context_id: $context_id, logical_key: f_key})
            OPTIONAL MATCH (f)-[:ABOUT]->(e:Entity)
            RETURN 
                f.logical_key AS fact_key,
                f.text AS text,
            ...
            """
            try:
                raw_nodes = self._hydra.read(cypher, {"fact_keys": fact_logical_keys, "context_id": context_id}, None)
            except Exception as e:
                logger.warning("HydraDB node query error: %s", e)
                raw_nodes = []
            ...
            chat_ttl_limit = int((query_epoch - timedelta(hours=24)).timestamp())
            ...
            if entities:
                ...
                """
                try:
                    path_res = self._hydra.read(path_cypher, {"entities": list(entities)}, None)
                    for row in path_res:
                        path = row.get("path", [])
                        if len(path) >= 3:
                            hops = len(path) // 2
            ...
            # 3. Traverse SUPERSEDES
            supersedes_cypher = """
            UNWIND $fact_keys AS f_key
            MATCH (f:Fact {context_id: $context_id, logical_key: f_key})-[:SUPERSEDES*1..5]->(old:Fact)
            RETURN f.logical_key AS newer, count(old) AS superseded_count
            """
            try:
                sup_res = self._hydra.read(supersedes_cypher, {"fact_keys": list(valid_fact_keys), "context_id": context_id}, None)
            except Exception as e:
                logger.debug("SUPERSEDES query skipped: %s", e)
    ...
    def _composite_scoring_and_synthesis(
        ...
        with timed_operation(logger, "retrieval.phase3.scoring_and_synthesis", {"facts_to_score": len(facts)}) as ctx:
            for f_id, fact in facts.items():
                ...
                fact.structural_score = (1.0 / hop_count) * min(path_count, 3) / 3.0
                fact.entity_boost = 0.5 / max(entity_fact_count, 1)
                
                fact.composite_score = (fact.semantic_score + fact.keyword_score + fact.structural_score + fact.entity_boost) / 3.0

            # Sort facts
            ranked = sorted(facts.values(), key=lambda f: f.composite_score, reverse=True)
            
            # Abstention check
            if not ranked or (ranked[0].semantic_score < 0.25 and ranked[0].structural_score == 0):
                ctx["abstention_triggered"] = True
                logger.info("Retrieval: Abstention triggered (max semantic score < 0.25 and zero structural score)")
            ...
            prompt = f"Using the following memory context, answer the user's question:\n\n{context_str}"
            with timed_operation(logger, "retrieval.phase3.reader_synthesis"):
                return self._llm.text_completion(prompt, question)
